...
После перезапуска backend live scan `ff1096ee-3a74-406c-a86b-85e4a9a04de1`: **32 lab users, 57 groups, 1 computer, 1 FGPP, 43 findings**, Critical **1**, High **26**, Medium **10**, Low **6**, AD Security Score **65/100**, 5,5 с (LDAP 200 мс, Event Log 5,3 с). Все источники, включая Security Event Log, имеют `pass`; Authentication: 986 событий, 50 ошибок пароля, **0 brute force/password spray findings**. Новая lab учётная запись event reader добавила один finding Password Never Expires. SQLite schema version 2 сохранила scan после backend restart; Dashboard, Authentication и CSV проверены.
...
| Security Event Log | отдельный SSH collector, события 4624/4625/4771/4776 | Обычный backend scan под `ir-event-reader`: `pass`, 986 normalized, 0 auth findings. Прямой read-only экспорт тем же reader подтвердил live 4624/4625/4771/4776. Для 4771 включён только failure audit Kerberos Authentication Service на DC. |
...
Утверждённый визуальный стиль, Dashboard/Risks/Accounts/Domain Policy/Connection сохранены. Добавлены поля в существующие панели, FGPP, настройки порогов, разделы Computers и Authentication. Старые API запросы и новые `/api/computers`, `/api/authentication`, `/api/checks`, `/api/config`, account/computer detail, CSV отработали после backend restart. Frontend build **PASS**. Chrome headless открыл семь страниц и карточку аккаунта на 1440/390 px без React exceptions и горизонтального переполнения; поиск Accounts/Risks проверен. После доработок browser QA проверил no scan, пустые findings, поиск без результатов, LDAP unavailable/wrong password/timeout, Event Log unavailable/partial/not_evaluated, loading, API error и неправильные Settings на desktop/mobile без React exceptions или горизонтального переполнения. CSV с UTF-8 BOM и `;` delimiter открыт в установленном Microsoft Excel: 14 колонок, 44 строки вместе с заголовком, русский текст читается корректно. Для обычных CSV parsers нужно указать delimiter `;`.
