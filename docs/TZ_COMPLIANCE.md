...
> **Актуальное состояние 2026-09-24:** см. [TZ_AUDIT_CURRENT.md](TZ_AUDIT_CURRENT.md). Приведённые ниже `LIVE VERIFIED` описывают ранее выполненные проверки. Сейчас VM контроллера домена находится в `Saved`; свежий live scan недоступен. Приватный Dashboard показывает сохранённый результат, а AD Gateway возвращает `503` для нового snapshot.
