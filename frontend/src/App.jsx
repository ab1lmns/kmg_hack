...
function Metric({ level, label, value, total, helper }) {
  ...
  return <div className={`metric metric-${level}`}>
    <div className="metric-top"><span className="metric-label">{label}</span><span className="metric-share" title="Доля от всех находок">{total > 0 ? Math.round(value / total * 100) : 0}%</span></div>
...
function DomainPolicy({ dashboard }) {
  ...
  const facts = [
    ['Минимальная длина', policy.min_password_length == null ? 'Нет данных' : policy.min_password_length === 0 ? '0 — не задана' : policy.min_password_length],
    ['Сложность пароля', policy.password_complexity == null ? 'Нет данных' : policy.password_complexity ? 'Включена' : 'Отключена'],
    ['История паролей', policy.password_history_count == null ? 'Нет данных' : policy.password_history_count === 0 ? '0 — не хранится' : policy.password_history_count],
    ['Максимальный возраст', policy.max_password_age_days == null ? 'Нет данных' : policy.max_password_age_days === 0 ? 'Без ограничения' : policy.max_password_age_days + ' дней'],
    ['Минимальный возраст', policy.min_password_age_days == null ? 'Нет данных' : policy.min_password_age_days + ' дней'],
    ['Порог блокировки', policy.lockout_threshold == null ? 'Нет данных' : policy.lockout_threshold === 0 ? '0 — отключена' : policy.lockout_threshold],
    ['Длительность блокировки', policy.lockout_duration_minutes == null ? 'Нет данных' : policy.lockout_duration_minutes + ' мин'],
    ['Окно блокировки', policy.lockout_observation_minutes == null ? 'Нет данных' : policy.lockout_observation_minutes + ' мин'],
  ]
  return <>
    <div className="section-head"><div><div className="eyebrow">Домен</div><h1>Парольная политика</h1><p>Настройки домена и найденные риски.</p></div><div className="detail-score"><span>Risk Score · 100 = хуже</span><strong>{policyAvailable ? dashboard.domain_policy_risk_score ?? 0 : '—'}<small>/100</small></strong></div></div>
    <section className="panel domain-policy-panel">
      <div className="panel-head"><div><h2>Параметры</h2><p>Проверяются длина и сложность пароля, а также блокировка. Для аккаунтов с FGPP учитывается их результирующая политика.</p></div></div>
      <dl className="facts domain-policy-facts">{facts.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>
    </section>
    <section className="panel fgpp-panel">
      <div className="panel-head"><div><h2>Дополнительные политики паролей</h2><p>Fine-Grained Password Policies · FGPP</p></div></div>
      {dashboard.fine_grained_policies?.length ? <div className="fgpp-grid">{dashboard.fine_grained_policies.map(item => <article className="fgpp-card" key={item.distinguished_name}>
        <div className="fgpp-card-head"><h3>{item.name}</h3><span>Приоритет {item.precedence}</span></div>
        <dl className="facts">
          <div><dt>Применяется к</dt><dd>{item.applies_to?.join(', ') || 'Нет данных'}</dd></div>
          <div><dt>Длина / история</dt><dd>{item.min_password_length} / {item.password_history_count}</dd></div>
          <div><dt>Сложность</dt><dd>{item.password_complexity ? 'Включена' : 'Отключена'}</dd></div>
          <div><dt>Возраст: мин / макс</dt><dd>{item.min_password_age_days ?? '—'} / {item.max_password_age_days ?? '—'} дн.</dd></div>
          <div><dt>Блокировка: порог / срок / окно</dt><dd>{item.lockout_threshold ?? '—'} / {item.lockout_duration_minutes ?? '—'} / {item.lockout_observation_minutes ?? '—'} мин.</dd></div>
        </dl>
