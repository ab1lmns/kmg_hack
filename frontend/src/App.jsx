...
export default function App() {
  ...
  const refresh = useCallback(async () => {
    const [summary, risks, users, ldap, history, delta] = await Promise.all([
      api('/dashboard'), api('/findings'), api('/accounts'), api('/connection/status'), api('/scans'), api('/scans/compare'),
    ])
    setDashboard(summary); setFindings(risks); setAccounts(users); setConnection(ldap); setScans(history); setComparison(delta)
