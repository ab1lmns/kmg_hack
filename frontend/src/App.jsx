...
function Dashboard({ dashboard, accounts, scans, comparison, onOpenAccount }) {
  ...
  return <>
    ...
    <div className="quick-stats"><span title="Пользователи в выбранной тестовой OU"><strong>{dashboard.total_users}</strong> аккаунтов проверено</span><span><strong>{dashboard.healthy_accounts ?? '—'}</strong> без находок</span><span title="Неактивность определяется заданным порогом; учитываются lastLogonTimestamp и точный вход на DC"><strong>{dashboard.inactive_accounts ?? '—'}</strong> неактивных</span><span title="Для этих аккаунтов вход не наблюдался; новые аккаунты не считаются неактивными до истечения порога"><strong>{accounts.filter(item => item.activity_status === 'never_observed').length}</strong> без наблюдаемого входа</span><span><strong>{dashboard.service_accounts}</strong> сервисных</span><span><strong>{dashboard.privileged_accounts}</strong> привилегированных</span><span><strong>{dashboard.disabled_accounts ?? '—'}</strong> отключённых</span><span><strong>{dashboard.locked_accounts ?? '—'}</strong> заблокированных</span><span><strong>{dashboard.expired_accounts ?? '—'}</strong> истёкших</span><span title="Компьютеры, прочитанные из домена"><strong>{dashboard.total_computers ?? '—'}</strong> компьютеров</span><span title={dashboard.source_status?.security_event_log === "pass" ? "Подозрительные паттерны в журнале Security, а не все события входа" : "Security Event Log не оценён"}><strong>{dashboard.source_status?.security_event_log === "pass" ? dashboard.auth_findings : "—"}</strong> сигналов риска входа</span><span><strong>{dashboard.finding_count}</strong> находок</span></div>
    ...
    {accounts.length > 0 && <p className="footnote">Показатели относятся к последнему завершённому сканированию. Изменения в AD появятся после повторного запуска. «Вход не наблюдался» не означает неактивность нового аккаунта до истечения заданного порога.</p>}
  </>
}
...
function AccountDetails({ account, onBack }) {
  return <><button className="back-button" onClick={onBack}>← К списку аккаунтов</button><div className="section-head"><div><div className="eyebrow">Карточка аккаунта</div><h1>{account.username}</h1><p>{account.display_name} · {account.department || 'Отдел не указан'}</p></div><div className="detail-score"><span>Risk Score · 100 = хуже</span><strong>{account.risk_score}<small>/100</small></strong><Badge level={account.risk_level}/></div></div>
    <div className="details-grid"><section className="panel detail-panel"><h2>Сведения</h2><p title="Risk Score: больше означает выше риск">Risk Score {account.risk_score}/100 · {severityLabels[account.risk_level]} · чем выше, тем больше риск</p><dl className="facts"><div><dt>Тип</dt><dd>{account.service_account ? 'Сервисный аккаунт' : 'Пользователь'}</dd></div><div><dt>Состояние</dt><dd>{account.enabled ? 'Включён' : 'Отключён'}</dd></div><div><dt>Последний вход</dt><dd>{account.last_logon ? formatDate(account.last_logon) : account.exact_last_logon ? `${formatDate(account.exact_last_logon)} (DC)` : account.activity_status === "never_observed" ? "Вход не наблюдался" : "Нет данных"}</dd></div><div><dt>Пароль установлен</dt><dd>{formatDate(account.password_last_set)}{account.password_last_set ? ` · ${daysAgo(account.password_last_set)} дней назад` : ''}</dd></div><div><dt>Владелец</dt><dd>{account.owner || 'Не указан'}</dd></div><div><dt>Привилегии</dt><dd>{account.privileged ? 'Есть' : 'Не найдены'}</dd></div></dl></section>
...
function Settings({ connection, onTest, testing, thresholds, onThresholdChange, onSave, saving }) {
  ...
  return <><div className="section-head"><div><div className="eyebrow">Подключение</div><h1>Источник Active Directory</h1><p>Параметры источника локальный backend читает из <code>backend/.env</code>. Секреты не передаются браузеру.</p></div></div>
    ...
    <section className="panel settings-panel"><h2>Как подключить сервер</h2><ol className="setup-list"><li>Скопируйте <code>backend/.env.example</code> в <code>backend/.env</code>.</li>{connection.connection_mode === 'AD Gateway' ? <li>Укажите публичный HTTPS Gateway URL и свой личный AD_GATEWAY_TOKEN.</li> : <><li>Укажите адрес контроллера, Base DN и учётную запись чтения.</li><li>Задайте <code>LDAP_PASSWORD</code> в закрытом окружении backend и перезапустите его.</li></>}<li>Проверьте соединение и запустите анализ Active Directory.</li></ol></section>
  </>
}
