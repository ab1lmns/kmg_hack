...
function Authentication({ authentication, dashboard }) {
  ...
  return <><div className="section-head"><div><div className="eyebrow">События</div><h1>Аутентификация</h1><p>События Security Event Log и сигналы подбора паролей.</p></div></div><section className="panel detail-panel"><h2>Источник событий</h2><dl className="facts"><div><dt>Статус</dt><dd>{status === 'pass' ? 'Проверен' : status === 'partial' ? 'Частичные данные' : status === 'error' ? 'Ошибка чтения' : 'Не оценён'}</dd></div><div><dt>Событий</dt><dd>{authentication?.events ?? 0}</dd></div><div><dt>Ошибки пароля</dt><dd>{authentication?.failed_bad_password ?? 0}</dd></div><div><dt>Сигналы</dt><dd>{authentication?.findings?.length ?? 0}</dd></div></dl>{status !== 'pass' && <p className="scope-note">{status === 'error' ? 'Не удалось прочитать Security Event Log. Проверьте подключение и права reader.' : status === 'partial' ? 'Собрана только часть журнала; выводы по атакам могут быть неполными.' : 'Источник Security Event Log не настроен или недоступен для оценки.'}</p>}</section><section className="panel detail-panel"><h2>Источники проверки</h2><dl className="facts">{Object.entries(dashboard.source_status ?? {}).map(([name, value]) => <div key={name}><dt>{name}</dt><dd>{value === 'pass' ? 'Проверен' : value === 'not_evaluated' ? 'Не оценён' : value === 'error' ? 'Ошибка чтения' : value === 'partial' ? 'Частичные данные' : String(value)}</dd></div>)}</dl></section><section className="panel finding-detail-panel"><h2>Обнаруженные сигналы</h2>{authentication?.findings?.length ? authentication.findings.map(item => <article className="finding-card" key={item.id}><Badge level={item.severity}/><h3>{item.title}</h3><p>{item.reason}</p><Evidence evidence={item.evidence}/></article>) : <p className="muted">{status === 'pass' ? 'Сигналов не найдено.' : status === 'partial' ? 'В собранной части журнала сигналов не найдено; полную проверку подтвердить нельзя.' : 'Без доступа к журналу вывод о наличии или отсутствии атак невозможен.'}</p>}</section></>
}
...
export default function App() {
  ...
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><div><strong>Identity Risk</strong><span>Security workspace</span></div></div><div className="nav-label">Рабочая область</div><nav aria-label="Основная навигация">{nav.map(item => <button key={item.key} onClick={() => setPage(item.key)} aria-current={targetPage === item.key || (targetPage === 'detail' && item.key === 'accounts') ? 'page' : undefined} className={`nav-link ${targetPage === item.key || (targetPage === 'detail' && item.key === 'accounts') ? 'active' : ''}`}><Icon name={item.icon} size={19}/><span>{item.label}</span></button>)}</nav><div className="sidebar-footer"><div className="sidebar-visual"><Icon name="shield" size={28}/><span>Видеть риски.<br/><strong>Защищать главное.</strong></span></div><div className="sidebar-bottom"><div><strong>{dashboard ? (dashboard.source === 'ldap' ? 'Active Directory' : 'Демо-режим') : 'Нет данных'}</strong><small>{dashboard ? 'Источник сканирования' : 'Ожидание сканирования'}</small></div><Icon name="server" size={18}/></div><div className="workspace-version">IDENTITY RISK RADAR <span>v0.1</span></div></div></aside>
    <main className="main"><header className="topbar"><div className="breadcrumb">Рабочая область <span>/</span> <strong>{page === 'detail' ? 'Карточка аккаунта' : nav.find(item => item.key === page)?.label}</strong></div><div className="top-actions"><label className="source-select"><span>Источник</span><select value={source} onChange={e => setSource(e.target.value)}><option value="demo">Демо</option><option value="ldap" disabled={!connection?.configured}>Active Directory</option></select><Icon name="chevron" size={15}/></label><button className="scan-button" disabled={scanning || (source === 'ldap' && connection?.password_required)} onClick={runScan}><Icon name="refresh" size={17}/>{scanning ? 'Чтение AD и анализ…' : 'Запустить анализ'}</button>{auth === 'signed-in' && <button className="logout-button" onClick={signOut}>Выйти</button>}</div></header>
      ...
      </> : <Empty title="Нет результатов" body="Запустите анализ, чтобы увидеть результаты."/>}</div>
    </main></div>
}
